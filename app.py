...
# Dictionary of Temperature (record method)
...
# Dictionary of Temperature (default method)
...
def welcome():
    ...
    return (
        ...
        f"If you want any other information, too bad.<br/>"
        f"Available Routes:<br/>"
        f"/api/v1.0/precipitation<br/>"
        f"/api/v1.0/stations<br/>"
        f"/api/v1.0/tobs<br/>"
        f"Interested in all weather from a given date forward? Use address below with date in YYYY-MM-DD format<br/>"
        f"/api/v1.0/start_from/<start1><br/>"
        f"Interested in weather between two dates? Use address below with dates as YYYY-MM-DD, YYYY-MM-DD<br/>"
        f"/api/v1.0/between/<start2><end>"
   )
...
def temperature():
    ...
    return jsonify(busiest_station_tobs_dict_default)

@app.route("/api/v1.0/start_from/<start1>")
def weather_from_start_date(start1):
    print(start1)
    #result = busiest_station_tobs_dict_default((date, tobs) for date, tobs in busiest_station_prcp_dict.items()) if date >= start1)
    #return f"Weather from {start1} to 2017-08-23<br/>"
    #return jsonify(busiest_station_prcp_dict.items() if date >= start1)
    #res = dict((k, test_dict[k]) for k in ['nikhil', 'akshat'] if k in test_dict) 
    #result = dict((k,busiest_station_tobs_dict_default[k]) for [k >= start1]
    #    if k in busiest_station_tobs_dict_default)
    #result = {key: busiest_station_tobs_dict_default[key] for key in busiest_station_prcp_dict.keys() & (key >= start1)}
   ...
   #date_entry = input('Enter a date in YYYY-MM-DD format')
    ...
    #start1 = datetime.date(year, month, day)
    #return "Say hey"

@app.route("/api/v1.0/between/<start2>,<end>")
def weather_from_start_to_finish_date(start2, end):
    print("")
    return f"Weather from {start2} to {end}<br/>"
#    date_entry = input('Enter a start date in YYYY-MM-DD format')
#    year, month, day = map(int, date_entry.split('-'))
#    start2 = datetime.date(year, month, day)
